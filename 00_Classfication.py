...
tokenizer = AutoTokenizer.from_pretrained("distilbert/distilbert-base-uncased") # 사용할 모델 선택

def preprocess_function(examples):
    return tokenizer(examples["text"]) # 토큰화, trucation(true)을 지우면 동적패딩 기능을 지움 => 토큰 길이를 맞춰줌


id2label = {0: "NEGATIVE", 1: "POSITIVE"}
label2id = {"NEGATIVE": 0, "POSITIVE": 1} # 포스트 프로세싱, 평가한 후 라벨 설정을 하는 단계
